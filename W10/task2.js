...
function TransferFunctionTexture()
{
    ...
    for ( var i = 0; i < resolution; i++ )
    {
        ...
        var S = i / 255;
	var R = 0;//Math.max( Math.cos( ( S - 1.0 )*Math.PI ), 0.0 );
	var G = 0;//Math.max( Math.cos( ( S - 0.5 )*Math.PI ), 0.0 );
	var B = 0;//Math.max( Math.cos( ( S )*Math.PI ), 0.0 );
	//var color = new THREE.Color( R,G,B );
	//cmap.push( [ alpha, '0x' + color.getHexString() ] );
        data[ 4 * i + 0 ] = color.x;
        data[ 4 * i + 1 ] = color.y;
        data[ 4 * i + 2 ] = color.z;
        data[ 4 * i + 3 ] = S;
